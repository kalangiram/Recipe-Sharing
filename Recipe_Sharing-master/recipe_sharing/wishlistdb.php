...
//            
            ...
            $sql = "INSERT INTO wishlist (shareby, username, recipe_name, recipe_type, recipe_description, file_name, uploaded_on) "
...
//            
            // if not product exists then we are newly inserting data into the "cart" table
            $sql = "DELETE FROM wishlist WHERE shareby = '$shareby' and username = '$userName' and file_name='$filename'";
            if ($conn->query($sql) === TRUE) {
                    echo '<script type="text/javascript">';
                    echo 'alert("Item deleted from Wishlist!");';
                    echo 'window.history.back();'; // Redirect to previous_page
                    echo '</script>';
                } else {
